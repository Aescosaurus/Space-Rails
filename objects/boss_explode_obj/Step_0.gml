if( zoom_time )
{
	...
	// else if( set_cam )
	{
		...
		camera_set_view_size( cam,
			lerp( cur_width,min_zoom_x,zoom_speed * dt ),
			lerp( cur_height,min_zoom_y,zoom_speed * dt ) )
		
		camera_set_view_pos( cam,
			lerp( camera_get_view_x( cam ),x,zoom_speed * dt ),
			lerp( camera_get_view_y( cam ),y,zoom_speed * dt ) )
	}
}